...
export function BarcodeScanner({
    ...
    useEffect(() => {
        const initializeSDKWrapper = async () => {
            try {
                ...
                const engineResourcePathValue =
                    engineResourcePath?.value || "https://unpkg.com/dynamsoft-javascript-barcode@9.6.42/dist/";
    ...
    const decodeImageBarcode = async (file: File) => {
        if (!isSDKLoaded) {
            setError(getText(textLoadingSDK, "Loading SDK..."));
            return;
        }
    ...
    // Helper function to get text from template with fallback
    const getText = (textTemplate: any, defaultValue: string): string => {
        if (textTemplate?.value && typeof textTemplate.value === "string" && textTemplate.value.trim() !== "") {
    ...
    return (
        <div className="barcode-reader">
            ...
            {preloadOnly ? (
                <div className="preload-section">{/* Preload mode - no visible UI, only console logging */}</div>
            ) : (
                            ...
                            <div className="camera-container" style={{ position: "relative" }}>
                                ...
                                >
                                    {isInitializing && (
                                        <div className="loading-overlay">
                                            <div className="loading-text">
                                                {getText(textPreparingCamera, "Preparing camera...")}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {scanCompleted && !continuousScan && (
                                        <div
                                            className="scan-completed-overlay"
                                            style={{
                                                position: "absolute",
                                                top: 0,
                                                left: 0,
                                        ...
                                        >
                                            ...
                                            >
                                                {getText(textStartAgain, "Start Again")}
                                            </button>
                                        </div>
                                    )}
                                    ...
                                    {!isScanning && !isInitializing && !scanCompleted && (
                                        <div className="camera-placeholder">
                                            <div className="placeholder-text">
                                                {getText(textCameraReady, "Camera ready")}
                                            </div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <p className="scan-info">
                                    {!isSDKLoaded
                                        ? getText(textLoadingSDK, "Loading SDK...")
                                        : isInitializing
                                        ? getText(textPreparingCamera, "Preparing camera...")
                                        : isScanning
                                        ? continuousScan
                                            ? getText(
                                                  textScanningContinuous,
                                                  "Scanning continuously... Point camera at barcode"
                                              )
                                            : getText(textPointCamera, "Point camera at barcode to scan")
                                        : scanCompleted && !continuousScan
                                        ? getText(
                                              textScanCompleted,
                                              'Scan completed! Click "Start Again" to scan another barcode.'
                                          )
                                        : error
                                        ? `Error: ${error}`
                                        : getText(textCameraReady, "Camera ready")}
                        ...
                        {enableImageDecode && (
                            <div className="image-decode-container">
                                <div className="upload-section">
                                    ...
                                    >
                                        {isDecodingImage
                                            ? getText(textDecoding, "Decoding...")
                                            : getText(textUploadImage, "Upload Image")}
                                    </button>
                                    <p className="upload-info">
                                        {isDecodingImage
                                            ? getText(textProcessingImage, "Processing image...")
                                            : !isSDKLoaded
                                            ? getText(textLoadingSDK, "Loading SDK...")
                                            : getText(
                                                  textUploadInstruction,
                                                  "Click to upload an image containing a barcode"
                                              )}
                                    </p>
                                </div>
                            </div>
